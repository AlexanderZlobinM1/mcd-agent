...
Symfony console boot, MCD deletes the active Composer/docroot `var/cache/prod`
or legacy `app/cache/prod` directory. It recreates the applicable directory
with the instance runtime owner/group and mode `0775`. The normal cache clear
and native plugin reload then generate a clean cache from the current plugin
filesystem.
...
The object contains `schema`, `operation`, `root`, `status`, `reset_paths`, and
`mode`.
