...
object. A successful result contains `schema`, `capability`, `status`,
`generated_at`, instance identity fields, `field_count`, `fields`, and an empty
`errors` array. Each field contains only:

- `alias`, `label`, `classification` (`native` or `custom`) and `field_type`;
...
the exact `mcd-contact-field-metadata-v1` capability and require MCD 1.2.29 or
newer before dispatching this report.
