...
class ReleaseApprovalTests(unittest.TestCase):
    ...
    def test_exact_live_approval(self):
        ...
        with patch.object(upgrade.urllib.request, "urlopen", return_value=response) as request:
            upgrade._require_release_approval(self.config, "7.1.3", "7.1.2")
        ...
        for error in (HTTPError("https://mcc.example", 409, "blocked", {}, None), URLError("offline"), TimeoutError(), ValueError()):
            with self.subTest(error=type(error).__name__), patch.object(upgrade.urllib.request, "urlopen", side_effect=error):
                with self.assertRaises(RuntimeError):
                    upgrade._require_release_approval(self.config, "7.2.1", "7.2.0")
    ...
    def test_denial_precedes_maintenance_and_package_changes(self):
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "blocked"):
                upgrade.run_upgrade_apply(config=self.config, root=install.root, mode="zip", yes=True, do_backup=False, with_system_upgrade=False, target_override="7.1.4")
            maintenance.assert_not_called()
            apply_zip.assert_not_called()

    def test_major_flag_does_not_automatically_choose_another_release_line(self):
        install = SimpleNamespace(root="/var/www/test", console_path="/var/www/test/bin/console", runtime="host")
        with (
            patch.object(upgrade, "_pick_install_record", return_value=install),
            ...
            patch.object(upgrade, "_latest_same_branch", return_value=None),
            patch.object(upgrade, "_require_release_approval") as approval,
        ):
            self.assertEqual(upgrade.run_upgrade_apply(config=self.config, root=install.root, mode="composer", yes=True, do_backup=False, with_system_upgrade=False, allow_major=True), 0)
